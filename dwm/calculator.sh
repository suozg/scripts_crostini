...
# --- Функція для запуску dmenu з динамічними кольорами ---
run_dmenu() {
    # Базові налаштування
    ...
    if [ -f "$HOME/.lightmode" ]; then
        # Світла тема
        dmenu -l "$LINES" -i -fn "$FONT" -nb "#eeeeee" -nf "#222222" -sb "#005577" -sf "#eeeeee" "$@"
    else
        # Стандартна (темна) тема
        dmenu -l "$LINES" -i -fn "$FONT" "$@"
    fi
}
...
# 1. Вивід запрошення 
CALC_INPUT=$(cat "$HISTORY_FILE" 2>/dev/null | run_dmenu -p "bc ($LAST_RESULT):")

# 2. Якщо користувач скасував введення, вихід
[ -z "$CALC_INPUT" ] && exit 0

# 3. Обробка та обчислення
if echo "$CALC_INPUT" | grep -q " = "; then
    EXPRESSION=$(echo "$CALC_INPUT" | awk -F ' = ' '{print $1}')
    # Якщо вибрано результат з історії, просто відкриваємо bc для нових розрахунків
    $HOME/.local/bin/st -e sh -c 'printf "\033]0;Калькулятор bc\007"; bc'
    exit 0
fi
...
if [ -n "$RESULT" ] && [ "$RESULT" != "." ]; then
    ...
    echo "$CLEAN_RESULT" | xclip -selection clipboard &
    exec "$0"
else
    exit 1
fi
